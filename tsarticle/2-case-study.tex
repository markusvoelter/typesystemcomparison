...
relation on entity types implied by the transitive closure of the \mykeyb{extends}
relation.
...
\begin{enumerate}
\item an attribute's initalization expression (if present) is conformant to its
 declared type (if specified);
\item the expression after the \emph{validate} clause is boolean;
\item text widgets do not refer to boolean entity attributes;
\item checkbox widgets refer only to boolean attributes.
\end{enumerate}
