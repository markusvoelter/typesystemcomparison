...
For a better IDE tooling and a more ``programming''-like style for
rules, Xsemantics rules are written in the other direction with respect
to standard deduction rules: the conclusion comes before the premises.
...
Java parameters) or output parameters (in that case the keyword
\verb|output| followed by the Java type of the output parameter can be used).
...
function invocation, thus one must specify the environment to pass to the rule,
...
In the premises one can assign values to the output parameters; and when 
another rule is invoked, upon return, the output arguments will have the values
...
\verb!key <- value!, 
where one can use any Xbase expression.
...
% etc.
Note that when an environment is passed to a rule with additional mappings,
actually a brand new environment is passed, thus the
current rule environment will not be modified; if a mapping already exists in
the current rule environment, in the brand new environment (and only there) the existing mapping
...
In the table we also show on the right the typical type deduction rules in a
possible formalization of our case study, to show how the specifications are
similar (apart from the order of the conclusion and the premises).  For typing a
literal (in the example a boolean literal) we write an axiom (since there is no premise) and the result is a \mytt{BooleanType} (created through the EMF factory for our language).  The rule for typing an \mytt{AttributeRef} can
...
\mytt{checkrule}; just like in Java validators for Xtext languages, one can have
