...
illustrated in the code below. Notice the type fo the \ic{Plus} expression.
...
\begin{lstlisting}[language=xts,float,label=lst:xts-widgets] 
...
We can now calculate the type of the \ic{FieldContent} expression, which has the
same type as the \ic{Attribute} to which the parent widget points. Since we have
defined the type of the \emph{Widget} to be the type of the reference \ic{Attribute}, we can
now use the following type specification for \ic{FieldContent}:
...
referenced entities are subtypes of each other \todo{Is this an example of
covariant subtyping?}. So instead of declaring a subtype relationship directly
