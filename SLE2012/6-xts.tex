...
Type system specifications  are structured into sections. Inside sections we can define \ic{typeof}
clauses. A \ic{typeof} clause defines how the type for a given EClass is
calculated, and can optionally specify constraints on the types of properties of
these metaclasses.
For instance, we specify that
the \ic{Type} metaclass and all its subclasses (hence the \ic{+}) is a clone of itself.
...
used as well (but not the other way around). In other words, \ic{IntType} is
more specialized.

The type of string and boolean literals is always the same, so it can
