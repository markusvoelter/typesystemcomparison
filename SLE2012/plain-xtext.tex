...
indicate that there is no expected type.
...
provided where a certain type is expected, also known as type conformance or
subtyping, which is an important part of a type system. 
The method \ic{is\-Assignable(left, right)} returns true if an element of
...
other methods.  As in Listing~\ref{lst:plain-type-provider1}, we must cache the
visited enities to avoid endless loops in case of malformed cyclic hierarchies.
